...
def ini_check():# 核查文件是否合格
    ...
    try:
        wtemplate = open('./resources/template.txt','r', encoding='UTF-8') 
        wtemplate.close()
        print("文件检查成功!")
    except:
        wtemplate = open('./resources/template.txt','a', encoding='UTF-8') 
        wtemplate.close()
        print("文件生成成功!")
    #3 检查是否有 dict.txt 分词库
    resources_check = os.path.exists('resources/dict.txt')
    if resources_check == False:
...
'''
    #print(setting_txt)
    resources_check = os.path.exists('resources/setting.txt')
    if resources_check == False:
        wsetting = open('./resources/setting.txt','w', encoding='UTF-8') 
...
def ini_setting_var(): #读取系统设置
    ...
    wsetting = open('./resources/setting.txt','r', encoding='UTF-8') # 文本形式，重写模式
        ...
        if platform.system() == 'Darwin':# Mac系统
            ...
            for file in os.listdir(path):  
                ...
                if os.path.isdir(file_path):  
                    ...
                    list_name.append('./resources/'+file_path[12:-4]+'.mp3')
                
    ls = []
    listdir('./resources',ls)#;print(ls)
    wsetting = open('./resources/setting_music_name.txt','w', encoding='UTF-8')
    for i in range(len(ls)):
        wsetting.write(ls[i]+'  '+str(ls[i])[12:-4]+'\n')
    wsetting.close()
    # 播放音乐
    a = False
    wsetting = open('./resources/setting.txt','r', encoding='UTF-8') # 文本形式，重写模式
...
def information_control_add(): # 信息添加 b1_2 按钮
    ...
    global b1_2_1 
    if platform.system() == 'Darwin':# Mac系统
        b1_2_1 = tk.Button(window, text="Back to last level", width=20, height=3,fg = COLOR_MB_FRONT,command=lambda:information_control_add_back())
    elif platform.system() == 'Windows':# Windows系统
        b1_2_1 = tk.Button(window, text="Back to last level", width=20, height=3,fg = COLOR_FRONT,bg = COLOR_SUB,command=lambda:information_control_add_back())
    note.pack()
    b1_2_1.pack()
    # 1.3主容器，左容器，右容器
...
def template_control_add_save(): # 模板添加 b2_4 按钮 小功能-保存信息
    if template_control_add_save_check(): 
      wtemplate = open('./resources/template.txt','a+',encoding='UTF-8') # 文本形式，追加写模式
...
def template_control_change():# 模板修改 b2_3 按钮
    ...
    def tem_control_change_item_save():
        ...
        if true:
            # 先把文件保存到工作区
            winfor = open('./resources/template.txt','r',encoding='UTF-8') # 文本形式，只读
            wincopy = open('./resources/inforchange.txt','w',encoding='UTF-8') # 文本形式，重写模式
            for line in winfor.readlines():
                wincopy.write(line)
            winfor.close()
            ...
            # 在进行文件重写
            wincopy = open('./resources/inforchange.txt','r',encoding='UTF-8') # 文本形式，只读
            winfor = open('./resources/template.txt','w',encoding='UTF-8') # 文本形式，重写模式
            for line in wincopy.readlines():
                i = 0;if_change = True
...
def setting_control():
    ...
    wtemplate = open('./resources/template.txt','r',encoding='UTF-8')
    wsetting = open('./resources/setting.txt','r',encoding='UTF-8')
...
def setting_do(item_text): # 设置生效函数
    ...
    if item_text[0] == 'music' and item_text[1] == 'on_or_off': # 设置音乐是否开启
        def setting_do_music_onoroff(event): #音乐开关
            if cmb3_1.get() == 'on':
                if item_text[2] == 'on':pass
                    ...
                    ini_music()#开音乐
                    # 先把文件保存到工作区
                    wsetting = open('./resources/setting.txt','r',encoding='UTF-8') # 文本形式，只读
                    ...
                    wsetting = open('./resources/setting.txt','w',encoding='UTF-8') # 文本形式，重写模式
        ...
        def setting_do_music_choose(event): #音乐开关
            ...
            ini_related_to_music()
        # 调用函数内容
        wsetting = open('./resources/setting_music_name.txt','r',encoding='UTF-8') # 文本形式，重写模式
        ls = []
        for line in wsetting.readlines():
