...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse login(LoginRequest dto) {
        ...
        RegisterResponse account = authAccountMapper.findById(mail.getUserId());
    ...
    public ProfileResponse profile(String userId) {
        RegisterResponse account = authAccountMapper.findById(Long.valueOf(userId));
        if (account == null) {
    ...
    public RegisterResponse register(RegisterRequest dto) {
        ...
        response.setPasswordHash(passwordEncoder.encode(dto.getPassword()));
        authAccountMapper.insert(response); // 插入后userId会自动设置到response对象
        ...
        profile.setUserId(response.getUserId()); // 直接使用认证ID作为用户ID
        ...
        mail.setUserId(response.getUserId()); // 关联认证ID
        mailMapper.insert(mail);

        // 6. 使用JWT工具类生成AccessToken
        String token = jwtUtils.generateAccessToken(response.getUserId().toString());
        response.setToken(token);

        // 7. 存储AccessToken到Redis
        tokenService.storeAccessToken(response.getUserId().toString(), token);
